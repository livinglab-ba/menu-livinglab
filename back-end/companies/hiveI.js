...
async function hiveComputerVisionI(req, res) {
  try {
    ...
    const sshCommands = [
      ...
      () => executeSSHCommand(`sshpass -p '${password}' ssh -o StrictHostKeyChecking=no pc-mini-03@10.146.42.53 'DISPLAY=:0.0 firefox http://10.146.42.12:9000/hiveInstitucional'`),
      () => executeSSHCommand(`sshpass -p '${password}' ssh -o StrictHostKeyChecking=no pc-mini-03@10.146.42.53 'DISPLAY=:0.0 firefox http://10.146.42.160/bluequeen/'`)
    ];
