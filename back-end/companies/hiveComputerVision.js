...
async function hiveComputerVision(req, res) {
  try {
    ...
    const sshCommands = [
      ...
      () => executeSSHCommand(`sshpass -p '${password2}' ssh -o StrictHostKeyChecking=no pcminia@10.146.42.53 'pkill firefox; DISPLAY=:0.0 firefox --noerrors --disable-session-crashed-bubble --disable-infobars$fobars --incognito http://10.146.42.70/bluequeen/'`)
    ];
