...
async function generalEnergy(req, res) {
  try {
    ...
    const sshCommands = [
      ...
      () => executeSSHCommand(`sshpass -p '${password2}' ssh -o StrictHostKeyChecking=no linuxlited@10.146.42.18 'DISPLAY=:0.0 firefox --noerrors --disable-session-crashed-bubble --disable-infobars$fobars --incognito http://10.146.42.12:9000/general-energy'`),
      () => executeSSHCommand(`sshpass -p '${password2}' ssh -o StrictHostKeyChecking=no pcminib@10.146.42.17 'DISPLAY=:0.0 firefox --noerrors --disable-session-crashed-bubble --disable-infobars$fobars --incognito https://park01-joselitol.us2.pitunnel.com/#!/0?socketid=sd-rj_mR2OSE8dA-AAB5'`)
    ];
